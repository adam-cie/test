...
class MSDTransformer(TransformerMixin):
    ...
    def fit(self, data, weights = None, objectives = None, expert_range = None):
        ...
        self.m = self.data.shape[1] # number of columns (-1, because we must exclude 0th column - alternatives)
        self.n = self.data.shape[0] # number of rows    (-1, because we must exclude 0th row - criteria)
        ...
        elif(type(objectives) == str): #when user will only give one value, it will be copied for all criteria
            self.objectives = np.repeat(objectives, self.m)
        elif(objectives == None):
    ...
    def checkInput(self):
        ...
        if(not all(item in ["min", "max"] for item in self.objectives)): 
            raise ValueError("Invalid value at 'objectives'. Use 'min', 'max', 'gain', 'cost', 'g' or 'c'.")

        if(self.expert_range != None):
            if(len(self.expert_range) != len(self.objectives)):
                raise ValueError("Invalid value at 'expert_range'. Length of should be equal to number of critetrias.")
            for col in self.expert_range:
                if(len(col) != 2):
                    raise ValueError("Invalid value at 'expert_range'. Every criterion has to have minimal and maximal value.")
                if(not all(type(item) in [int, float]  for item in col)):
                    raise ValueError("Invalid value at 'expert_range'. Expected numerical value (int or float).")
                if(col[0]>col[1]):
                    raise ValueError("Invalid value at 'expert_range'. Minimal value "+col[0]+" is bigger then maximal value "+col[1]+".")
    ...
    def normalizeData(self, data):
        ...
        if self.expert_range is None:
            ...
            for col in data.columns:
                data[col] = (data[col] - self.expert_range[c][0])/(self.expert_range[c][1]-self.expert_range[c][0])
                c += 1
    ...
    def topsis(self):
        ...
        if type(self.agg_fn) == str:
            if self.agg_fn == 'I':
                self.data['AggFn'] = 1 - np.sqrt((1-self.data['Mean'])*(1-self.data['Mean'])+(self.data['Std']*self.data['Std']))
                topsis_val = self.data['AggFn']
            elif self.agg_fn == 'A':
                self.data['AggFn'] = np.sqrt(self.data['Mean']*self.data['Mean']+(self.data['Std']*self.data['Std']))
                topsis_val = self.data['AggFn']
            elif self.agg_fn == 'R':
                self.data['AggFn'] = (np.sqrt(self.data['Mean']*self.data['Mean']+(self.data['Std']*self.data['Std'])))/(((1 - np.sqrt((1-self.data['Mean'])*(1-self.data['Mean'])+(self.data['Std']*self.data['Std'])))-1)*(-1) + (np.sqrt(self.data['Mean']*self.data['Mean']+(self.data['Std']*self.data['Std']))))
                topsis_val = self.data['AggFn']
        else:
